...
def _format_simple_response(
    ...
    数据超过 10 条时，只返回摘要提示，详细数据由前端表格展示。
    ...
    # 数据超过 10 条时，返回简短摘要，详细数据由前端表格展示（F-01）
    if total > 10:
        return f"查询成功，共返回 **{total}** 条数据。详细结果请见下方表格，导出 Excel 可获取完整数据。"
...
async def analyzer_node(state: AgentState) -> AgentState:
    ...
    # 检查是否有权限错误（plan 为空但没有数据）
        ...
        if not all_data:
            ...
            # 获取原始数据量（summarize_data_for_llm 可能已降维）
            ...
            if is_simple_query(plan, all_data) and original_count <= 10:
                logger.info(
                ...
                # 大数据量查询：返回简短摘要，详细数据由前端表格展示
                logger.info(
                    f"[AnalyzerNode] Large data query ({original_count} rows), returning summary"
                )
                analysis_report = (
...
def _extract_all_data(data_context: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for key, result in data_context.items():
        ...
        logger.debug(f"[AnalyzerNode] Processing {key}: status={result.get('status')}, success={result.get('success')}, is_success={is_success}, has_data={bool(result.get('data'))}")

        if is_success and result.get("data") is not None:
            data = result["data"]
            ...
            # 处理单个字典数据（包括Alpha Vantage原始响应）
                ...
                # 检查嵌套结构：{"data": [...]}（如盗版用户查询API）
                if "data" in data and isinstance(data["data"], list):
                    nested_rows = data["data"]
                    logger.info(f"[AnalyzerNode] Extracting {len(nested_rows)} rows from nested data in {key}")
